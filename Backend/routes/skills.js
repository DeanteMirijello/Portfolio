...
router.post("/types", [body("name").isString().trim().notEmpty().withMessage("name is required")], (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  return addSkillType(req, res, next);
});

router.put("/types/:name", [body("newName").isString().trim().notEmpty().withMessage("newName is required")], (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  return renameSkillType(req, res, next);
});

router.put("/types/:name/titles", [
...
router.delete("/types/:name", (req, res, next) => deleteSkillType(req, res, next));
